	...
	{#if data.user}
		<section>
			...
			{#if data.user.avatarUrl}
				<img src={data.user.avatarUrl} alt="{data.user.username} avatar" />
			{/if}
			<ul>
				<li>Level: {data.user.level}</li>
				<li>Military Rank: {data.user.militaryRank}</li>
				{#if data.user.country}
					<li>Country: {data.user.country.name} ({data.user.country.code})</li>
				{/if}
				{#if data.user.party}
					...
					{#each data.user.transactions as transaction (transaction.id)}
						<li>
							{#if transaction.__typename === 'MarketTransaction'}
								Market: {transaction.money} for {transaction.item.itemCode}
							...
							{:else if transaction.__typename === 'CaseTransaction'}
								Case: {transaction.case} ({transaction.item.itemCode})
							{:else if transaction.__typename === 'TradeTransaction'}
								Trade: {transaction.quantity}x {transaction.itemCode} for {transaction.money}
								{#if transaction.sellerCountry}
									({transaction.sellerCountry.name} → {transaction.buyerCountry?.name})
								{/if}
							...
							<ul>
								...
								<li>
									...
									{#if company.region.initialCountry.code !== company.region.country.code}
										(originally {company.region.initialCountry.code})
									{/if}
								</li>
								<li>
									...
									{#if company.employees.length > 0}
										<ul>
											{#each company.employees as employee (employee.user.id)}
												<li>
													{employee.user.username}: wage {employee.wage}, fidelity {employee.fidelity}
												</li>
											{/each}
										</ul>
									{/if}
					...
					{#each data.user.skillSnapshots as snapshot (snapshot.since)}
						<details>
							...
							<ul>
								...
								<li>Entrepreneurship: {snapshot.set.entrepreneurship}</li>
								<li>Production: {snapshot.set.production}</li>
								<li>Critical Chance: {snapshot.set.criticalChance}</li>
								<li>Critical Damages: {snapshot.set.criticalDamages}</li>
			...
			{:else}
				<ul>
					{#each data.user.countryHistory as entry (entry.at)}
						<li>{entry.at}: {entry.country.name} ({entry.country.code})</li>
					{/each}
				</ul>
			{/if}
		</section>
					...
					{#each data.user.financeReports as report (report.dayStart)}
						<details>
							...
							<ul>
								...
								<li>Value Dismantled: {report.valueDismantled}</li>
				...
				<ul>
					{#each data.user.flipEvents as event (event.at + event.itemCode)}
						<li>
							...
							— Bought: {event.buyCost}, Sold: {event.sellRevenue}, Profit: {event.profit}
							(held for {(event.heldMs / 1000 / 60 / 60).toFixed(1)} hours)
						</li>
					{/each}
				</ul>
			{/if}
		...
		<section>
			...
			<ul>
				<li>Total Damage: {data.user.battleParticipation.totalDamage}</li>
				<li>Battles Participated: {data.user.battleParticipation.battlesParticipated}</li>
				<li>Negative Damage: {data.user.battleParticipation.negativeDamage}</li>
				<li>Own Country Battles: {data.user.battleParticipation.ownCountryBattles}</li>
				<li>Own Country Participated: {data.user.battleParticipation.ownCountryParticipated}</li>
				<li>MU Order Battles: {data.user.battleParticipation.muOrderBattles}</li>
				<li>MU Order Participated: {data.user.battleParticipation.muOrderParticipated}</li>
