	...
	<section>
		...
		{#if data.mu.region}
			<ul>
				...
				<li>Initial Country Code: {data.mu.region.initialCountry?.code ?? "None"}</li>
			</ul>
		{:else}
			<p>No region data.</p>
		{/if}
		...
		{:else}
			<ul>
				{#each data.mu.members as member (member.id)}
					<li>{member.username} (ID: {member.id}) - {member.avatarUrl}</li>
				{/each}
			</ul>
		{/if}
	</section>
				...
				{#each data.mu.orderChanges as change, index (`oc-${index}`)}
					<li>
						...
						<ul>
							...
							<li>Defender Region ID: {change.battle.defenderRegion?.id ?? "None"}</li>
							<li>Defender Region Name: {change.battle.defenderRegion?.name ?? "None"}</li>
							<li>Defender Region Country Code: {change.battle.defenderRegion?.country?.code ?? "None"}</li>
							<li>Defender Region Initial Country Code: {change.battle.defenderRegion?.initialCountry?.code ?? "None"}</li>
						</ul>
					</li>
				{/each}
			</ul>
		...
		{:else}
			<ul>
				{#each data.mu.ownerHistory as entry, index (`oh-${index}`)}
					<li>
						{entry.at}: {entry.owner.username} (ID: {entry.owner.id}) - {entry.owner.avatarUrl}
					</li>
				{/each}
			</ul>
		{/if}
		...
		{:else}
			<ul>
				{#each data.mu.wealthReports as report (report.dayStart)}
					<li>
						{report.dayStart}: totalDamage={report.totalDamage}, totalWealth={report.totalWealth}
					</li>
				{/each}
			</ul>
		{/if}
