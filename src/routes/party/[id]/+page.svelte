    ...
    {#if data.party}
        <section>
            ...
            {#if data.party.avatarUrl}
                <img
                    src={data.party.avatarUrl}
                    alt="{data.party.name} avatar"
                />
            {/if}
            ...
            {#if data.party.country}
                <p>
                    Country: {data.party.country.name} ({data.party.country
                        .code})
                </p>
            {/if}
            ...
            {:else}
                <ul>
                    {#each data.party.leaderHistory as entry (entry.at)}
                        <li>
                            {entry.at}: {entry.leader.username} (ID: {entry
                                .leader.id})
                        </li>
                    {/each}
                </ul>
            ...
            {:else}
                <ul>
                    {#each data.party.ethicsHistory as entry (entry.at)}
                        <li>
                            {entry.at}: Unethical {entry.ethics.unethical},
                            Militarism {entry.ethics.militarism}, Isolationism {entry
                                .ethics.isolationism}, Imperialism {entry.ethics
                                .imperialism}, Industrialism {entry.ethics
                                .industrialism}
                        </li>
                    {/each}
                </ul>
                ...
                <ul>
                    {#each data.party.wealthReports as report (report.dayStart)}
                        <li>
                            ...
                            — Members: {report.memberCount}, Damage: {report.totalDamage},
            ...
            {:else}
                {#each data.party.moneyFlows as flow (flow.dayStart)}
                    <details>
                        ...
                        <ul>
                            <li>
                                In Equipment: {flow.inEquipment} (inside party: {flow.inEquipmentInsideParty},
                                outside alliance: {flow.inEquipmentOutsideAlliance},
                                same country outside party: {flow.inEquipmentSameCountryOutsideParty},
                                same alliance cross border: {flow.inEquipmentSameAllianceCrossBorder})
                            </li>
                            <li>
                                Out Equipment: {flow.outEquipment} (inside party:
                                {flow.outEquipmentInsideParty}, outside
                                alliance: {flow.outEquipmentOutsideAlliance},
                                same country outside party: {flow.outEquipmentSameCountryOutsideParty},
                                same alliance cross border: {flow.outEquipmentSameAllianceCrossBorder})
                            </li>
                            <li>
                                In Items: {flow.inItems} (inside party: {flow.inItemsInsideParty},
                                outside alliance: {flow.inItemsOutsideAlliance},
                                same country outside party: {flow.inItemsSameCountryOutsideParty},
                                same alliance cross border: {flow.inItemsSameAllianceCrossBorder})
                            </li>
                            <li>
                                Out Items: {flow.outItems} (inside party: {flow.outItemsInsideParty},
                                outside alliance: {flow.outItemsOutsideAlliance},
                                same country outside party: {flow.outItemsSameCountryOutsideParty},
                                same alliance cross border: {flow.outItemsSameAllianceCrossBorder})
                            </li>
                            <li>
                                In Wages: {flow.inWages} (inside party: {flow.inWagesInsideParty},
                                outside alliance: {flow.inWagesOutsideAlliance},
                                same country outside party: {flow.inWagesSameCountryOutsideParty},
                                same alliance cross border: {flow.inWagesSameAllianceCrossBorder})
                            </li>
                            <li>
                                Out Wages: {flow.outWages} (inside party: {flow.outWagesInsideParty},
                                outside alliance: {flow.outWagesOutsideAlliance},
                                same country outside party: {flow.outWagesSameCountryOutsideParty},
                                same alliance cross border: {flow.outWagesSameAllianceCrossBorder})
                            </li>
                        </ul>
