...
<h1>Items</h1>
<p>This page shows item market reports and hourly average prices from the last 24 hours.</p>
				...
				<li>
					...
					{#if item.report}
						<ul>
							...
							<li>avgWeighted24h: {item.report.avgWeighted24h}</li>
					...
					{:else}
						<p>hourlyAvgPrices: [{item.hourlyAvgPrices.join(", ")}]</p>
					{/if}
				</li>
			{/each}
		</ul>
