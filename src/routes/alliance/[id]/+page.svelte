			...
			{:else}
				<ul>
					{#each data.alliance.countries as country (country.id)}
						<li>
							{country.name} ({country.code}) - {country.userCount} users
						</li>
					{/each}
				</ul>
			{/if}
			...
			{:else}
				<ul>
					{#each data.alliance.topDamage as damageEntry, index (damageEntry.user.id)}
						<li>
							#{index + 1} - {damageEntry.user.username} (ID: {damageEntry.user.id})
							<ul>
					...
					{#each data.alliance.moneyFlows as flow (flow.dayStart)}
						<li>
							...
							<ul>
								...
								<li>In Equipment In Alliance: {flow.inEquipmentInAlliance}</li>
								<li>Out Equipment In Alliance: {flow.outEquipmentInAlliance}</li>
								<li>In Equipment Outside Alliance: {flow.inEquipmentOutsideAlliance}</li>
								<li>Out Equipment Outside Alliance: {flow.outEquipmentOutsideAlliance}</li>
								<li>In Items In Alliance: {flow.inItemsInAlliance}</li>
								<li>Out Items In Alliance: {flow.outItemsInAlliance}</li>
								<li>In Items Outside Alliance: {flow.inItemsOutsideAlliance}</li>
								<li>In Wages In Alliance: {flow.inWagesInAlliance}</li>
								<li>Out Wages In Alliance: {flow.outWagesInAlliance}</li>
								<li>In Wages Outside Alliance: {flow.inWagesOutsideAlliance}</li>
								<li>Out Wages Outside Alliance: {flow.outWagesOutsideAlliance}</li>
							</ul>
						</li>
					{/each}
				</ul>
